...
from documents.services.importance import LABELS, classify_change_row  # noqa: E402
...
def print_confusion_matrix(confusion: dict[str, dict[str, int]]) -> None:
    ...
    for true_label in labels:
        row = [true_label] + [
            str(confusion[true_label][pred_label]) for pred_label in labels
        ]
        print(fmt_row(row))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate rule-based importance classifier on CSV dataset."
    )
    parser.add_argument(
        "--dataset",
    ...
    for row in rows:
        ...
        enriched_row["triggered_rules"] = json.dumps(
            prediction.triggered_rules, ensure_ascii=False
        )
    ...
    accuracy, macro_f1, per_label, confusion = compute_metrics(
